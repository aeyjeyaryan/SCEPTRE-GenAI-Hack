...
#     """Fetch full content from a URL using Crawl4AI, falling back to BeautifulSoup if it fails."""
#     headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    
#     # Try Crawl4AI first
#     try:
#         async with AsyncWebCrawler() as crawler:
#             result = await crawler.arun(url=url, headers=headers, bypass_cache=True)
#             if result.success and result.extracted_content:
#                 return result.extracted_content
#             else:
#                 logger.warning(f"Crawl4AI failed for {url}, falling back to BeautifulSoup")
#     except Exception as e:
#         logger.error(f"Crawl4AI error for {url}: {str(e)}, falling back to BeautifulSoup")
...
#         logger.error(f"BeautifulSoup error fetching content from {url}: {e.response.status_code} {e.response.reason_phrase} for url '{e.request.url}'")
...
## USING Beautiful Soup for now since deploying crawl4ai on render is not possible as of now.
## We will use crawl4ai when deploying on GCP
...
async def fetch_document_content(url: str) -> str:
    """Fetch full content from a URL using BeautifulSoup."""
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    ...
    try:
        async with httpx.AsyncClient(follow_redirects=True) as client:
            ...
            # Extract text from paragraphs, divs, and other content elements
            ...
            # If no content found from specific elements, try to get all text
            if not content:
        ...
        logger.error(f"HTTP error fetching content from {url}: {e.response.status_code} {e.response.reason_phrase}")
        if e.response.status_code in (301, 302):
            logger.error(f"Redirect location: '{e.response.headers.get('Location', 'Unknown')}'")
        return ""
    except Exception as e:
